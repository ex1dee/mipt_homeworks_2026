...
def show_paged_chats(sessions: List[Any], current_idx: Optional[int], page: int = 1,
                     page_size: int = 10) -> None:
    total_pages = (len(sessions) - 1) // page_size + 1
    table = Table(title=f"Список ваших чатов ({page}/{total_pages})", title_style="bold cyan", expand=True)
